// Terraform module for File Server VM
...
resource "proxmox_vm_qemu" "file_server" {
  ...
  tags = "fileserver,infrastructure,${(length(keys(var.config)) > 0 && try(var.config.organization.short_name, null) != null) ? var.config.organization.short_name : var.organization_short}"
  ...
  connection {
    ...
    host = (length(keys(var.config)) > 0 && try(var.config.network.file_server_ip, null) != null) ? var.config.network.file_server_ip : var.server_ip
    timeout = "5m"
  }
...
output "vm_id" { value = proxmox_vm_qemu.file_server.id }
output "vm_ip" { value = (length(keys(var.config)) > 0 && try(var.config.network.file_server_ip, null) != null) ? var.config.network.file_server_ip : var.server_ip }
output "vm_name" { value = proxmox_vm_qemu.file_server.name }
